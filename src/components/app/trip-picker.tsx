 ...
 * other city in the directory as a trip. A free student sees the same chip,
 * and tapping another city opens the value-first upsell rather than a locked
 * select that does nothing.
 */
export function TripPicker({
  ...
  const go = (slug: string) => {
    ...
    if (slug === homeSlug) next.delete("city");
  ...
  return (
    <div className="relative mb-3">
      ...
      >
        ...
        {!unlocked ? <Lock className="size-3 text-ink-400" /> : null}
      </button>

      {open ? (
        <div className="absolute top-full left-0 z-30 mt-2 w-80 rounded-2xl bg-white p-3 shadow-[var(--shadow-lift)] ring-1 ring-ink-950/8">
          ...
          <ul className="mt-2 max-h-64 space-y-0.5 overflow-y-auto">
            {home ? (
              <li>
                <button type="button" onClick={() => go(home.slug)} className={cn("flex w-full items-center justify-between rounded-lg px-3 py-2 text-left hover:bg-paper-2", currentSlug === home.slug && "bg-signal-soft")}>
            ...
            {filtered.filter((city) => city.slug !== homeSlug).map((city) => (
              <li key={city.slug}>
                <button type="button" onClick={() => go(city.slug)} className={cn("flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left hover:bg-paper-2", currentSlug === city.slug && "bg-signal-soft")}>
          ...
          {!unlocked ? <p className="mt-2 text-[0.75rem] text-ink-500">Trips are part of Pro. Picking a city shows what it unlocks.</p> : null}
        </div>
      ) : null}
    </div>
  );

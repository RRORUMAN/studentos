 ...
 * legal requirement, and the note under the list says so.
 * ============================================================================
 */
    ...
    <div className="rounded-2xl bg-white p-5 shadow-[var(--shadow-raise)] ring-1 ring-ink-950/5 sm:p-6">
      ...
      >
        <div className="mt-5 flex flex-wrap items-baseline justify-between gap-2">
          <h4 className="text-display-xs text-ink-950">{stage.tagline}</h4>
          <SampleTag />
        </div>

        <ol className="mt-3 flex flex-col">
          {stage.tasks.map((task, index) => (
            <li key={task.label} className="flex gap-3 border-b border-ink-100 py-3 last:border-b-0">
              ...
              <span className="min-w-0 flex-1">
                <span className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                  <span className="text-[0.9375rem] font-medium text-ink-950">{task.label}</span>
                  {task.official ? (

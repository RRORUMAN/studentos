 ...
 * ============================================================================
 */
export function AnyoneDownDemo() {
  ...
  return (
    <div className="mx-auto w-full max-w-[28rem] rounded-2xl bg-white p-5 shadow-[var(--shadow-lift)] ring-1 ring-ink-950/6 sm:p-6">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h4 className="mt-1.5 text-display-xs text-ink-950">
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        <p className="text-[0.875rem] font-medium text-ink-700">
          {complete
            ? "Group is full"
            : `Need ${total - joined} ${total - joined === 1 ? "person" : "people"}`}
        </p>
      ...
      <div className="mt-2 flex gap-1.5">
        {Array.from({ length: total }).map((_, slot) => (
          <motion.span
            ...
            className={cn("h-1.5 flex-1 rounded-full", slot < joined ? "bg-pulse" : "bg-ink-200")}
          />
        ))}
      </div>
          ...
          {anyoneDownDemo.joiners.slice(0, joined).map((person, index) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-xs text-ink-500">{person.campus}</span>
              </span>
        ...
        {joined === 0 ? (
          <li className="flex flex-1 items-center gap-3 rounded-lg border border-dashed border-ink-200 px-3 py-4">
            ...
            <p className="text-[0.8125rem] leading-snug text-ink-500">
              Post the plan and your city answers. Press the button to watch.
            </p>
          </li>
        ) : null}
      </ul>
      ...
      <AnimatePresence initial={false}>
        {complete ? (
          ...
          >
            ...
            <p className="text-xs text-ink-500">Shared plan: {anyoneDownDemo.planLine}</p>
          </motion.div>
        ) : null}
      </AnimatePresence>

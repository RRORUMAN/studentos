...
export const toolMeta: Record<ToolName, { label: string; detail: string }> = {
  ...
  read_budget: { label: "read_budget", detail: "Safe today, safe this week, what is committed, which categories are drifting." },
  ...
  switch (name) {
    ...
    case "read_budget": {
      ...
      return {
        ...
        cards: [],
        figures: [
          { label: "Safe today", value: fmt(money$.reading.safeTodayCents), tone: money$.reading.safeTodayCents > 0 ? "good" : "bad" },
          { label: "Safe this week", value: fmt(Math.max(0, money$.week.targetCents - money$.week.spentCents)) },

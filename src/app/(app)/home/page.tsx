...
/**
 * ============================================================================
 * HOME — your day in StudentOS
 * ----------------------------------------------------------------------------
 * One hierarchy, top to bottom:
 *
 *   1. Who and where            greeting, city, day
 *   2. What can I afford        Safe today · Safe this week · one sentence
 *   3. What should I do         quick actions, chosen from the situation
 *   4. What matters today       the brief
 *   5. What is on right now     the live strip, only when something is
 *   6. For you                  one mixed feed with a reason on every card
 *   7. Who wants to join        people and plans, opt-in only
 *   8. Where am I in the move   arrival or leaving, when it applies
 *
    ...
    <div className="page max-w-3xl py-6 sm:py-8">
      ...
      <header className="mb-5">
        ...
        <h1 className="mt-1.5 text-display-sm text-ink-950">
          {greeting(now.getHours())}, {viewer.profile.displayName}.
        </h1>
      </header>

      {/* ---- stage-specific lead ------------------------------------------ */}
      {stage === "before-arrival" ? (
        <Countdown days={viewer.stage.daysUntilArrival ?? 0} cityName={viewer.city.name} />
      ) : null}
      ...
      <div className="mt-6">
        <AskBar
          ...
          safeTodayLabel={
            data.money.unset ? null : `${money(data.money.reading.safeTodayCents / 100, where)} today`
          }
        />
      </div>
      ...
      <div className="mt-8 space-y-9">
        ...
        {(stage === "first-24h" || stage === "first-week" || stage === "first-month") &&
        data.arrivalTotal > 0 ? (
        ...
        <section aria-labelledby="for-you-heading">
          <SectionHead
            title="For you"
            hint="Events, places, deals and people — each with the reason it is here."
            href="/discover"
            hrefLabel="Discover"
          />
          {data.feed.length === 0 ? (
            <Empty
              line="Nothing matches you closely enough yet. Add a few interests and it fills in."
            ...
            <ul className="space-y-3">
              {data.feed.map((item) => (
                <li key={`${item.kind}-${item.id}`}>
                  <FeedCard item={item} where={where} now={now} />
                </li>
              ))}
            </ul>
          )}
        ...
        {social ? (
          <section aria-labelledby="people-heading">
            <SectionHead
              title="People and plans for you"
              hint="Students near your campus and interests. Opt-in, campus and interests only — never location."
              href="/anyone-down"
              hrefLabel="Anyone down?"
            />

            {data.openInvites.length === 0 && data.people.length === 0 ? (
              <Empty
                line="Start with your campus. Join a plan, or post one, and this fills in."
                action="Post a plan"
                href="/anyone-down"
              />
            ) : (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-0.5 block text-[0.8125rem] text-ink-500">
                        {invite.going} in · {Math.max(0, invite.capacity - invite.going)} spots ·{" "}
                    ...
                    <span className="shrink-0 rounded-full bg-signal px-3 py-1.5 text-[0.8125rem] font-semibold text-ink-950">
                      Join
                    </span>
                  </Link>
                ))}
        ...
        <Link
          href="/pulse"
          className="flex items-center gap-4 rounded-xl bg-white p-4 shadow-[var(--shadow-flat)] ring-1 ring-ink-950/6 transition-shadow hover:shadow-[var(--shadow-raise)]"
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-[0.9375rem] font-semibold text-ink-950">
              What students in {viewer.city.name} are saying
            </span>
            <span className="mt-0.5 block text-[0.8125rem] text-ink-500">
              Prices, deals and questions, from people who live here.
            </span>
          </span>
...
function Countdown({ days, cityName }: { days: number; cityName: string }) {
  return (
    <Link
      href="/arrival"
      className="mt-4 flex items-center gap-4 rounded-2xl bg-flow-soft/70 p-5 ring-1 ring-flow-deep/15 transition-shadow hover:shadow-[var(--shadow-raise)]"
    >
      <MascotArt state="thinking" className="size-14 shrink-0" />
      <span className="min-w-0 flex-1">
        <span className="font-mono text-micro uppercase tracking-[0.1em] text-flow-deep">
          Arriving in
        </span>
        <span className="tnum mt-0.5 block font-display text-[1.75rem] leading-none font-semibold text-flow-deep">
          {days} {days === 1 ? "day" : "days"}
        </span>
        <span className="mt-1.5 block text-[0.875rem] text-ink-700">
          Some of this is much easier before you land in {cityName}. Open your move plan.
        </span>
      </span>
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-baseline justify-between gap-3">
          ...
          <span className="tnum font-mono text-[0.75rem] text-ink-500">
            {done}/{total}
          </span>
        </span>
...
function LeavingPrompt({ days }: { days: number }) {
  return (
    <Link
      href="/leaving"
      className="mt-4 flex items-center gap-4 rounded-2xl bg-pulse-soft/60 p-5 ring-1 ring-pulse-deep/15 transition-shadow hover:shadow-[var(--shadow-raise)]"
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="mt-0.5 block text-[0.875rem] text-ink-600">
          Cancel what renews, sell what you cannot take, do the things you kept meaning to.
        </span>
      </span>
